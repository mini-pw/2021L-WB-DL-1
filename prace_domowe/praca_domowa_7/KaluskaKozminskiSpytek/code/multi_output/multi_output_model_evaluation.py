...
print('hej')
clf_accuracy = eval_metrics.accuracy(y_pred=clf_pred, y=te_labels)
clf_recall = eval_metrics.recall(y_pred=clf_pred, y=te_labels)
clf_precision = precision_score(te_labels, clf_pred)
clf_f1score = f1_score(te_labels, clf_pred)

output_folder = 'multi_output/'
if not os.path.exists(output_folder):
    os.makedirs(output_folder)
file_perf = open(output_folder + 'performances_clf.txt', 'w')
file_perf.write("ACCURACY(pixel accuracy): " + str(clf_accuracy)
                + "\nSENSITIVITY: " + str(clf_recall)
                + "\nPRECISION: " + str(clf_precision)
                + "\nF1 Score: " + str(clf_f1score)
                )
file_perf.close()
