...
def _conv_block(x, filterNum, name, regularization = None):
    """
    """
    ...
    x1 = Conv3D(filterNum, (3, 3, 3), kernel_regularizer=regularization, strides=(1, 1, 1), use_bias=False, padding='same', name=name + '_1_conv')(x1)
    x1 = BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=name + '_1_bn')(x1)
    x1 = Activation('relu', name=name + '_1_relu')(x1)
    x1 = Conv3D(filterNum, (3, 3, 3), kernel_regularizer=regularization, strides=(1, 1, 1), padding='same', use_bias=False, name=name + '_2_conv')(x1)
    x = Concatenate(axis=bn_axis, name=name + '_concat')([x, x1])
    return x

def _transit_block(x, filterNum, name, regularization = None):
    """
    """
    bn_axis = -1 if K.image_data_format() == 'channels_last' else 1
    x = BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=name + '_trans_bn')(x)
    x = Activation('relu', name=name + '_trans_relu')(x)
    x = Conv3D(filterNum, (1, 1, 1), kernel_regularizer=regularization, strides=(1, 1, 1), use_bias=False, padding='same', name=name + '_trans_conv')(x)
    return x
...
name = 'block_0'
"""
def _denseBlock(x, blocks, name, regularization = None):
    for ie, i in enumerate(blocks):
        x = _conv_block(x, i, name=name+'_'+str(ie), regularization=regularization)
    return x
...
def myDenseNetv2Edited(input_shape, regularization=None, regularization_weight=0.01):
    """
    """
    bn_axis = -1 if K.image_data_format() == 'channels_last' else 1

    __regu = None
    if regularization in ["l1","l2"] and type(regularization_weight) is float:
        if regularization == "l1":
            __regu = l1(l1=regularization_weight)
        elif regularization =="l2":
            __regu = l2(l2=regularization_weight)

    input_tensor = Input(shape=input_shape)  # 48, 240, 360, 1
    x = Conv3D(16, (3, 3, 3), strides=(1, 2, 2), kernel_regularizer=__regu, use_bias=False, padding='same', name='block0_conv1')(input_tensor)  # [48, 120, 180]
    x = BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block0_bn1')(x)
    x = Activation('relu', name='block0_relu1')(x)
    x = Conv3D(16, (3, 3, 3), kernel_regularizer=__regu, strides=(1, 1, 1), use_bias=False, padding='same', name='block0_conv2')(x)  # [48, 120, 180]

    x = _denseBlock(x, [16, 16], 'block_11', __regu)  # [48, 120, 180]
    x = _transit_block(x, 16, 'block13', __regu)  # [48, 120, 180]
    x = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), padding='same')(x)  # [24, 60, 90]

    x = _denseBlock(x, [24, 24, 24], 'block_21', __regu)  # [24, 60, 90]
    x = _transit_block(x, 24, 'block23', __regu)  # [24, 60, 90]
    x = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), padding='same')(x)  # [12, 30, 45]

    x = _denseBlock(x, [32, 32, 32, 32], 'block_31', __regu)  # [12, 30, 45]
    x = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), padding='same')(x)  # [6, 15, 23]
    x = BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block_final_bn')(x)
    x = Activation('relu', name='block_final_relu')(x)

    ##############above are bae####################
    x = _denseBlock(x, [32, 32], 'EGFR_block_11', __regu)
    x = MaxPooling3D((1, 2, 2), strides=(1, 2, 2), padding='same')(x)  # [6, 8, 12, 64]
    x = _transit_block(x, 64, 'EGFR_block_12', __regu)  # [6, 8, 12, 64]
    x = GlobalAveragePooling3D()(x)
    x = Dense(1, kernel_regularizer=__regu, activation='sigmoid', name='EGFR_global_pred')(x)
