...
from keras.layers import Input, Dropout, Conv3D
...
def conv_block(y, nb_filter, filter_size, atrous_rate=(1, 1)):
    x = Conv2D(nb_filter, filter_size, dilation_rate=atrous_rate, padding='same')(y)
    x = bn_block(x)
    x = Activation('relu')(x)
    x = Conv2D(nb_filter, filter_size, dilation_rate=atrous_rate, padding='same')(x)
    x = bn_block(x)
    x = Activation('relu')(x)
